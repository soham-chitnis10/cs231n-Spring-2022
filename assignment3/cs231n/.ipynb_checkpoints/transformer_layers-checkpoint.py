...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        i = torch.arange(0,max_len).unsqueeze(1)
        a = torch.arange(0,embed_dim,2)
        exp = torch.pow(torch.tensor([10000]),a*(-1/embed_dim))
        pe[:,:,0::2] = torch.sin(i*exp)
        pe[:,:,1::2] = torch.cos(i*exp)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        softmax = self.attn_drop(F.softmax(scores,dim=-1))
        output = torch.matmul(softmax, value).transpose(1, 2).reshape((N, S, -1))
