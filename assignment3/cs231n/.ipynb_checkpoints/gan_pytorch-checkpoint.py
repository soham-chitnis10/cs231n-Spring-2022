...
def discriminator_loss(logits_real, logits_fake):
    ...
    N = logits_real.shape
    loss = bce_loss(logits_real,torch.ones(N).type(dtype).squeeze())+ bce_loss(logits_fake,torch.zeros(N).type(dtype).squeeze())
...
def generator_loss(logits_fake):
    ...
    N = logits_fake.shape
    loss = bce_loss(logits_fake,torch.ones(N).type(dtype).squeeze())
